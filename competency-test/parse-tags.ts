...
// Inlined from utils/STATIC-VARIABLES.ts
...
// Inlined from utils/parsers/product-tag-helpers.ts
type ParsedShippingTag = {
  shippingType: ShippingOptionsType;
  shippingCost: number;
};

function parseShippingTag(tag?: string[]): ParsedShippingTag | undefined {
...
// Inlined from components/utility-components/display-monetary-info.tsx
function calculateTotalCost(product: { price: number; shippingCost?: number }): number {
  let total = product.price;
  total += product.shippingCost ? product.shippingCost : 0;
  return total;
}
...
// parseTags() function
export const parseTags = (productEvent: NostrEvent): ProductData | undefined => {
