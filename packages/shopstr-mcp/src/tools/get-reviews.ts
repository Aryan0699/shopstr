...
function eventReferencesSeller(event: NostrEvent, sellerPubkey: string): boolean {
...
async function resolveProductAddressFromProductId(
  ...
  const productEvent = mergeAndDeduplicateProducts(relayResult.events).find(
...
export async function handleGetReviews(
  ...
  if (productId && !productAddress) {
    const resolved = await resolveProductAddressFromProductId(productId, context);
